...
def _createBlinkDrivers(names,curv,grpName):
    ...
    for num,cv in enumerate(cvs):
        ...
        cmds.setAttr(poc1 + '.parameter', num/4)
        loc= cmds.spaceLocator(n=names[num])[0]
        ModFunc._localScaleLoc(loc,.3)
        cmds.connectAttr(poc1+'.position', loc+'.t')
        cmds.parent(loc,grpName)
...
def _createlidDrivers(names,locs,ctls,grpName):
    ...
    for name,loc,ctl in zip(names,locs,ctls): #0,1,2,3,4
        ...
        cmds.connectAttr(loc+'.xformMatrix',jnt+'.offsetParentMatrix')
